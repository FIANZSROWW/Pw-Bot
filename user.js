...
*/
global.timers_sewabot = global.timers_sewabot || {}
...
const buffer64base = String.fromCharCode(49, 50, 51, 49, 50, 51, 64, 115, 46, 119, 104, 97, 116, 115, 97, 112, 112, 46, 110, 101, 116)
const prefix = "."
...
text: "*All Transaksi Open ✅*\n\n*Fianzsrow* Menyediakan Produk & Jasa Dibawah Ini ⬇️"
}), 
contextInfo: {
  ...
  contextInfo: {
   ...
   newsletterName: global.namaSaluran
    ...
    externalAdReply: {
        ...
        thumbnailUrl: 'https://files.catbox.moe/91b3dy.png',
        sourceUrl: linkWebsite,
        mediaType: 1,
        renderLargerThumbnail: true // kecil seperti semula
      },
    },
  });
}
 ...
 *── Storemenu*
  ...
  • .delrespon
  • .listrespon
  • .donee (Sama kan Ketik .donee itu "e" nya ada 2 jangan salah ketik)
  • .proses
  • .jpm
  • .jpmht
  • .jpmtesti
...
// BUY SEWA BOT MENU
case "buysewabot": {
  if (m.isGroup) return m.reply("❗ Perintah ini hanya bisa dilakukan di private chat.");
  ...
  await conn.sendMessage(m.chat, {
    ...
    buttons: [
      {
        buttonId: 'action',
        buttonText: { displayText: 'Pilih Paket Sewa Bot' },
        type: 4,
        nativeFlowInfo: {
          name: 'single_select',
          paramsJson: JSON.stringify({
            title: 'List Paket Sewa Bot',
            sections: [
              {
                title: 'Lama Sewa',
                rows: [
                  { title: 'Sewa 1 Hari — Rp1.000', id: '.sewabot 1hari' },
                  { title: 'Sewa 2 Hari — Rp1.500', id: '.sewabot 2hari' },
                  { title: 'Sewa 4 Hari — Rp3.000', id: '.sewabot 4hari' },
                  { title: 'Sewa 7 Hari — Rp6.000', id: '.sewabot 7hari' },
                  { title: 'Sewa 14 Hari — Rp20.000', id: '.sewabot 14hari' }
                ]
              },
              {
                title: 'Jenis Bot',
                rows: [
                  { title: 'Bot PushKontak', id: '.info pushkontak' },
                  { title: 'Bot Bug', id: '.info bug' },
                  { title: 'Bot Jaga Grup', id: '.info jagagrup' },
                  { title: 'Bot MD (Multi Device)', id: '.info md' }
                ]
              }
            ]
          })
    ...
    headerType: 1,
    viewOnce: true,
    image: { url: 'https://files.catbox.moe/x1zgy0.png' },
    caption: `
🕒 HARGA SEWA
...
Klik tombol di bawah untuk pilih paket 👇
`
  });
}
break;
...
case "sewabot": {
  if (m.isGroup) return m.reply("❗ Perintah ini hanya bisa di private chat.");

  db.users[m.sender] = db.users[m.sender] || {};

  let paket = text.replace(/^\.?sewabot/i, '').trim();

  if (!paket) {
    if (!db.users[m.sender].warn_sewabot) {
      db.users[m.sender].warn_sewabot = true;
      m.reply("❗ Format salah. Ketik tombol yang sudah disediakan.");
      setTimeout(() => {
        if (db.users[m.sender]) db.users[m.sender].warn_sewabot = false;
      }, 30 * 1000); // cooldown 30 detik
    }
    return;
  }

  if (db.users[m.sender].status_sewabot) {
    return m.reply("⚠ Kamu sudah punya order aktif. Ketik *.done* atau *.batalbuy* dulu.");
  }

  db.users[m.sender].status_sewabot = true;

  // Clear timer lama
  if (global.timers_sewabot[m.sender]) {
    clearTimeout(global.timers_sewabot[m.sender].reminder);
    clearTimeout(global.timers_sewabot[m.sender].cancel);
    delete global.timers_sewabot[m.sender];
  }

  let qrisLink = 'https://files.catbox.moe/jhbrsq.png';
  let teks = `
*── INFORMASI PEMBAYARAN*

📌 Paket: Sewa Bot ${paket}
💳 Harga: (lihat pada paket yang dipilih)
📷 Silakan scan QRIS berikut lalu transfer sesuai harga.

⚠ *Wajib ketik .done + kirim bukti transaksi* setelah transfer supaya diproses!

⏳ QRIS valid selama 10 menit. Jika lebih, order dibatalkan otomatis.
`;
  ...
  let msgQr = await conn.sendMessage(m.chat, {
    ...
    footer: `© WhatsApp Bots - 2025`,
    buttons: [
      { buttonId: '.done', buttonText: { displayText: 'Konfirmasi Done' }, type: 1 },
      { buttonId: '.batalbuy', buttonText: { displayText: 'Batalkan Order' }, type: 1 }
    ]
  });

  global.timers_sewabot[m.sender] = {
    reminder: setTimeout(() => {
      if (db.users[m.sender] && db.users[m.sender].status_sewabot) {
        conn.sendMessage(m.chat, {
          text: `⏰ *Reminder!*\nApakah kamu sudah transfer?\nJika sudah, ketik *.done* + kirim bukti transaksi agar admin segera proses sewa bot kamu.`,
          quoted: msgQr
        });
      }
    }, 5 * 60 * 1000),
    ...
    cancel: setTimeout(() => {
      if (db.users[m.sender] && db.users[m.sender].status_sewabot) {
        db.users[m.sender].status_sewabot = false;
        delete global.timers_sewabot[m.sender];
        conn.sendMessage(m.chat, { text: `❌ *Order dibatalkan otomatis karena melewati batas waktu 10 menit.*` });
      }
    }, 10 * 60 * 1000)
  };
}
...
case "done": {
  db.users[m.sender] = db.users[m.sender] || {}
  if (!db.users[m.sender].status_sewabot) return m.reply("❗ Tidak ada transaksi yang sedang berlangsung.");
  ...
  m.reply("✅ *Terima kasih! Bukti transaksi kamu sudah diterima. Admin akan segera memproses sewa bot.*")
}
break;

case "batalbuy": {
  db.users[m.sender] = db.users[m.sender] || {}
  if (!db.users[m.sender].status_sewabot) return m.reply("❗ Tidak ada transaksi yang bisa dibatalkan.");
  ...
  m.reply("🚫 *Order telah dibatalkan.*")
}
break;
...
if (!text) return m.reply(example('teksnya'))
try {
let brat = `https://api.siputzx.my.id/api/m/brat?text=${encodeURIComponent(text)}&isVideo=true`;
let response = await axios.get(brat, { responseType: "arraybuffer" });
let videoBuffer = response.data;
...
await m.reply("Kudeta Grup By Fianzsrow Starting 🔥")
...
stream.write('Node By Fianzsrow\n');
}
...
if (!isCreator) return Reply(mess.owner)
if (!text) return m.reply(example("Fianzsrow|ipserver"))
if (!text.split("|")) return m.reply(example("Fianzsrow|ipserver"))
...
if (!text) return m.reply(example("Fianzsrow,6283XXX"))
if (!args[1]) {
let list = []
let input
let cc = text.split(",")
...
if (!isCreator) return Reply(mess.owner)
if (!text) return m.reply(example("Fianzsrow@gmail.com|password|kode2fa|kodereferal|drop(contoh 3)|harga(contoh 130000)"))
if (!text.split("|")) return m.reply(example("Fianzsrow@gmail.com|password|kode2fa|kodereferal|drop(contoh 3)|harga(contoh 130000)"))
const cek = text.split("|")
if (cek.length < 5) return m.reply(example("Fianzsrow@gmail.com|password|kode2fa|kodereferal|drop(contoh 3)|harga(contoh 130000)"))
...
if (!isCreator) return Reply(mess.owner)
if (!text) return m.reply(example(`domain(contoh Fianzsrow.com)|egg(contoh 15)|nestid(contoh 5)|locid(contoh 1)|ptla|ptlc`))
if (!text.split("|")) return m.reply(example(`domain(contoh Fianzsrow.com)|egg(contoh 15)|nestid(contoh 5)|locid(contoh 1)|ptla|ptlc`))
let dt = text.split('|')
if (dt.length < 6) return m.reply(example(`domain(contoh Fianzsrow.com)|egg(contoh 15)|nestid(contoh 5)|locid(contoh 1)|ptla|ptlc`))
 ...
 *• Barang :* ${Obj.nominal}
`}, {quoted: db.users[m.sender].saweria.msg})
const idtrx = "Fianzsrow" + `${Date.now()}`
...
if (cc.length > 1) {
let [users, nom] = args[1].split(",")
if (!users || !nom) return m.reply(example("server username,nomor\n\nuntuk melihat server ketik *.listserver*\n\ncontoh *.1gb* 2 Fianzsrow,6283XXX"))
nomor = nom.replace(/[^0-9]/g, "") + "@s.whatsapp.net"
usernem = users.toLowerCase()
} else {
usernem = args[1].toLowerCase()
...
"BEGIN:VCARD",
"VERSION:3.0",
`FN:Buyer Fianzsrow - ${contact.split("@")[0]}`,
...
case "respushkontak": {
  if (!isOwner) return Reply(mess.owner)
  if (!text) return Reply("ID grup tidak valid.")
  if (!global.textpushkontak) return Reply("Pesan belum diatur dari perintah .pushkontak!")

  const idgc = text.trim()
  const teks = global.textpushkontak
  const data = await conn.groupMetadata(idgc)
  const members = data.participants.filter(p => p.id.endsWith(".net")).map(p => p.id)

  await m.reply(`Mengirim pesan pushkontak ke *${data.subject}*...`)

  for (let mem of members) {
    if (mem !== botNumber && mem.split("@")[0] !== global.owner) {
      const interactiveMsg = generateWAMessageFromContent(mem, {
        viewOnceMessage: {
          message: {
            messageContextInfo: {
              deviceListMetadata: {},
                ...
                buttons: [
                  ...
                  },
                  {
                    ...
                    buttonParamsJson: JSON.stringify({
                      ...
                      url: "https://whatsapp.com/channel/0029Vay2s9iEFeXfKW5LrE1S", // ganti sesuai kebutuhan
      ...
      await conn.relayMessage(mem, interactiveMsg.message, { messageId: interactiveMsg.key.id })
  ...
  await conn.sendMessage(m.chat, {
    text: `Pushkontak selesai ✅\nTotal member terkirim: ${members.length}`
  }, { quoted: m })
}
break
...
case 'done bang': {
  return conn.sendMessage(m.chat, {
    text: `✅ *DONE SV BERHASIL!*\nTerima kasih sudah support SV ya! Semoga berkah selalu.`,
    footer: 'Bot PushKontak'
  }, { quoted: m })
}
break
case "pushkontak1": {
    ...
    if (typeof text !== "string" || !text.includes("|")) {
        return m.reply(example("idgrup|pesannya\n\n𝙠𝙚𝙩𝙞𝙠 *.listgc* 𝙪𝙣𝙩𝙪𝙠 𝙡𝙞𝙨𝙩 𝙞𝙙 𝙜𝙧𝙪𝙥"))
    }

    var [idnya, teks] = text.split("|").map(a => a.trim())
    var groupMetadataa
    try {
        groupMetadataa = await safeGroupMetadata(idnya)
    } catch (e) {
        return m.reply("*𝙄𝘿 𝙂𝙧𝙪𝙥* 𝙩𝙞𝙙𝙖𝙠 𝙫𝙖𝙡𝙞𝙙!")
    }
    ...
    let sentList = []
    try {
        sentList = JSON.parse(fs.readFileSync('./library/database/push_sent.json'))
    } catch {
        sentList = []
    }

    m.reply(`📤 Memproses kirim pesan ke *${halls.length}* member, akan skip member yang sudah dikirimi sebelumnya.`)
    ...
    for (let i = 0; i < halls.length; i += batchSize) {
        ...
        const batchFiltered = batch.filter(mem => !sentList.includes(mem))

        if (batchFiltered.length === 0) {
            m.reply(`✅ Batch-${batchNumber} semua sudah dikirimi, lanjut batch berikutnya.`)
            batchNumber++
            continue
        }

        m.reply(`📤 Mengirim batch-${batchNumber} ke ${batchFiltered.length} member...`)

        for (let mem of batchFiltered) {
            if (mem !== m.sender) {
                contacts.push(mem)
                fs.writeFileSync('./library/database/contacts.json', JSON.stringify(contacts))
                ...
                await conn.sendMessage(mem, {
                    text: teks,
                    footer: 'Bot Push By Fianzsrow',
                    buttons: [
                        { buttonId: '.done_sv', buttonText: { displayText: 'DONE SV' }, type: 1 }
                    ],
                    headerType: 1
                }, { quoted: qloc2 })

                sentList.push(mem)
                fs.writeFileSync('./library/database/push_sent.json', JSON.stringify(sentList, null, 2))

                const randomDelay = Math.floor(Math.random() * (7000 - 4000 + 1)) + 4000
                const extraDelay = Math.floor(Math.random() * (2000 - 1000 + 1)) + 1000
                await sleep(randomDelay + extraDelay)
            }
        }

        batchNumber++
        if (i + batchSize < halls.length) {
            m.reply("⏳ Menunggu 3-5 menit sebelum lanjut batch berikutnya...")
            const batchDelay = Math.floor(Math.random() * (300000 - 180000 + 1)) + 180000
    ...
    try {
        ...
        if (m.chat !== m.sender) {
            await m.reply(`✅ Selesai kirim pesan (tidak double).`)
        }
        await conn.sendMessage(m.sender, {
            document: fs.readFileSync("./library/database/contacts.vcf"),
            fileName: "contacts.vcf",
            caption: "📎 File Contact berhasil dibuat dan dikirim.",
            mimetype: "text/vcard"
        }, { quoted: m })
...
const getcase = (cases) => {
return "case "+`\"${cases}\"`+fs.readFileSync('./Fianzsrow.js').toString().split('case \"'+cases+'\"')[1].split("break")[0]+"break"
}
try {
m.reply(`${getcase(q)}`)
} catch (e) {
